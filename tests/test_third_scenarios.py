...
def test_downloading_complete(browser):
    ...
    download_page = DownloadPage(main_page.browser)  # тут name download_page не корректен
    download_page.click_footer_download_link()
...
def test_size_downloaded_file(browser):
    ...
    assert real_file_size == download_page.get_expires_size_file()
